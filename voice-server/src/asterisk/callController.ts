...
type AsteriskEventPayload = Record<string, unknown>
...
async function resolveCallByEvent(payload: AsteriskEventPayload) {
  const byUniqueId = await findCallByUniqueId(
    String(payload.uniqueId ?? payload.unique_id ?? payload.Uniqueid ?? "")
  )
...
export async function handleAsteriskEvent(
  ...
  const caller = payload.caller as { number?: string } | undefined
  ...
  switch (eventType) {
    ...
    case "call.inbound":
      return createInboundCall({
        ...
        didNumber: firstString(
          payload.didNumber,
          payload.did_number,
          payload.CallerIDDNID,
          payload.DNID
        ),
    ...
    case "call.answered": {
      ...
      const updated = await setCallAnswered(
        String(call.id),
        agentId,
        payload.answeredAt ? String(payload.answeredAt) : null
      )
    ...
    case "call.ended": {
      ...
      const updated = await finalizeCall(
        String(call.id),
        status,
        payload.endedAt ? String(payload.endedAt) : null
      )
    ...
    case "recording.completed": {
      const call = await resolveCallByEvent(payload)
      if (!call?.id || !payload.filePath) return null
...
export async function transferCall(callId: string, endpoint: string) {
  const call = await getCallById(callId)
...
export async function assignAgentToCall(callId: string, agentId: string) {
