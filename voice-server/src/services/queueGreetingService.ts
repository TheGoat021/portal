...
const greetingCache = new Map<string, QueueGreetingAsset>()
const execFileAsync = promisify(execFile)

export type QueueGreetingAsset = {
  mediaKey: string
  wavPath: string
  alawPath: string
  ulawPath: string
...
function buildGreetingName(queueSlug: string, audioUrl: string) {
...
async function convertGreetingFile(
  inputFile: string,
  outputFile: string,
  format: "alaw" | "mulaw"
) {
...
export async function resolveQueueGreetingAsset(input: {
  ...
  const asset: QueueGreetingAsset = {
