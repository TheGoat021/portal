...
import { createAgent, listAgents, listAvailableAgents, updateAgentStatus } from "../services/agentService.js";
...
agentsRouter.put("/:id/status", async (req, res, next) => {
    try {
        ...
        res.json(await updateAgentStatus(req.params.id, status, req.body?.currentCallId ?? null));
    }
    catch (error) {
        next(error);
    }
