...
function splitAudioFrames(buffer) {
    const frames = [];
...
function buildRtpPacket(playback, payload) {
...
function startStreaming(playback) {
...
export function attachExternalMediaRemote(playbackId, remote) {
    const playback = playbacks.get(playbackId);
    if (!playback || playback.remote)
        return false;
...
export async function stopExternalMediaPlayback(playbackId) {
    const playback = playbacks.get(playbackId);
    if (!playback)
        return;
